...
echo ""
echo "$(tput bold)$(tput setaf 7)Read Me:$(tput sgr0) The administrator's e-mail address will used for sending alerts generated by the firewall and other system programs. It will also be used to create WordPress administrator users."
echo ""
read -p "Enter the administrator's e-mail address: " ADMIN_EMAIL_ADDRESS
if [[ "$ADMIN_EMAIL_ADDRESS" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$ ]]
then
    echo "$ADMIN_EMAIL_ADDRESS is in valid format."
else
    echo "$(tput bold)$(tput setaf 3)Warning:$(tput sgr0) $ADMIN_EMAIL_ADDRESS appears to be in an invalid e-mail format."
fi
echo ""
...
case "$CLOUDFLARE_YESNO" in
  y|Y ) read -p "Enter your Cloudflare e-mail address (press enter to use administrator's e-mail address): " CLOUDFLARE_EMAIL_ADDRESS
        if [[ "$CLOUDFLARE_EMAIL_ADDRESS" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$ ]]
        then
          echo "$CLOUDFLARE_EMAIL_ADDRESS is in valid format."
        else
          echo "$(tput bold)$(tput setaf 3)Warning:$(tput sgr0) $CLOUDFLARE_EMAIL_ADDRESS appears to be in an invalid e-mail format."
        fi
