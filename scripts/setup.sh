...
echo ""
echo "$(tput bold)$(tput setaf 2)Step 2 of 6:$(tput sgr0) Set up an administrator account"
echo ""
...
echo ""
echo "$(tput bold)$(tput setaf 2)Step 3 of 6:$(tput sgr0) Configure Cloudflare API usage"
echo ""
...
while [ "$WORDPRESS_ADMIN_PASSWORD" != "$WORDPRESS_PASSWORD_CHECK" ]; do 
    echo ""
    echo "Passwords did not match. Try again."
    echo ""
    read -s -p "Enter the WordPress administrator password: " WORDPRESS_ADMIN_PASSWORD
    echo ""
    read -s -p "Re-enter the WordPress administrator password: " WORDPRESS_PASSWORD_CHECK
done
echo ""
...
if [[ -z "$WORDPRESS_ADMIN_EMAIL" ]]; then
        ...
        until [[ "$WORDPRESS_ADMIN_EMAIL" =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$ ]]; do
          echo "$WORDPRESS_ADMIN_EMAIL is an invalid e-mail address format. Try again."
          echo ""
          read -p "Enter the WordPress administrator's e-mail (press enter to use $ADMIN_EMAIL_ADDRESS): " WORDPRESS_ADMIN_EMAIL
          if [[ -z "$WORDPRESS_ADMIN_EMAIL" ]]; then
            WORDPRESS_ADMIN_EMAIL=$ADMIN_EMAIL_ADDRESS
...
bash cloudflare-domains.sh $CLOUDFLARE_EMAIL_ADDRESS $CLOUDFLARE_API_KEY $APPS_SETTINGS $CLOUDFLARE_ALL_WEBSITES $GITHUB_ID $CLOUDFLARE_WP_YESNO $WORDPRESS_ADMIN_EMAIL  $URL_FUTURE $DEFAULT_WORDPRESS_INSTALL_TITLE $WORDPRESS_ADMIN_USER $WORDPRESS_ADMIN_PASSWORD
fi
echo ""
echo ""
