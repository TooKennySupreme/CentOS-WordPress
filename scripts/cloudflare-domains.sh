...
do
        ...
        create_srv_status=($( php -f /usr/local/src/gigabyteio/cloudflare/new-srv-record.php $1 $2 $i ${priority[$n]} ${service[$n]} $srvname $protocol $weight ${port[$n]} ${target[$n]} ))
                ...
                echo "* $(tput setaf 6)Adding SRV record for $i \(TTL: $srv_ttl Service: ${service[$n]} $protocol Priority: ${priority[$n]} Weight: $weight Port: ${port[$n]} Target: ):  $create_srv_status$(tput sgr0)"
